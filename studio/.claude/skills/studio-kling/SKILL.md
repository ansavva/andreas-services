...
The family:
- **`studio-prompt`** — authors the prompt. `--engine kling-replicate` compiles a
...
chose. See [`studio-scene`](../studio-scene/SKILL.md).
...
`studio-prompt`'s JSON serialization is validated against **Seedance**, following
