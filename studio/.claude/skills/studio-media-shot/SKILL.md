...
| `studio-media-image` | rendering one still |
| `studio-media-seedance` / `studio-media-kling` | rendering one video |
| `studio-media-s3` | the run store and format conversion |
...
Prose, not structured JSON — `studio-media-prompt`'s schema is camera/action/scene
...
Via `studio-media-prompt --engine kling-replicate` with `"start_image": true`. The
...
`studio-media-image` came to be written down.
...
A single still with no motion → just `studio-media-image`. A clip from an existing
