...
/**
 * The copy-into-a-character form, in a drawer beside the output it is about.
 *
 ...
 * output has to stay on screen while it is filled in. Lifted whole from the run
 * page when that page went: the feed and the opened run both open it, and the
 ...
 * succeeds** — it copies the output into a folder and describes the copy; the
 * run is not touched by any of it.
 */
...
/**
 * Whether this output can be copied into a character.
 *
 * **Images only.** What a character is kept and matched against is pictures, so
 * a clip is not offered — the CLI says the same thing by resolving `--from-run`
 * output nodes against its image extension set. Decided on `content_type`,
 * which the API sends off the stored row, rather than on the filename: the
 * extension is a label a rename can change and the type is what was measured
