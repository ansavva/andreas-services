...
without `--write`.
  ...
  uv run add_model.py openai/gpt-image-2              # propose, write nothing
...
uv run $STUDIO models show {key}          # entry + LIVE input schema
uv run $STUDIO run --model {key} \\
...
def main() -> int:
    ...
    if REG.by_model_id(args.model):
        existing = REG.by_model_id(args.model)
        die(f"{args.model} is already registered as {existing['key']!r}.\n"
            f"       edit models.json directly, then: studio.py models refresh {existing['key']}")

    token = RA.load_token()
    try:
        props, schemas = MS.fetch(args.model, token)
    except MS.SchemaError as e:
        die(str(e))
    if not props:
        die(f"{args.model} has no published input schema — cannot onboard it.")

    entry, notes = infer(args.model, props, schemas, readme(args.model, token))
    key = args.key or args.model.split("/", 1)[1]
