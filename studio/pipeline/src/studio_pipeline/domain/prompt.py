...
# Which override goes where in the structured object. Declared rather than
# written out as three near-identical blocks of `if x is not None`.
...
def prompt(source, action, aspect_ratio, audio, camera_movement, camera_shot, compact, duration, emit, engine, json_, lens_mm, lighting, negative, no_audio, resolution, scene, seed, start_image, strict, style, subject):
    ...
    apply_overrides(
        obj, subject=subject, action=action, scene=scene, style=style,
        lighting=lighting, audio=audio, negative=negative,
        start_image=start_image, camera_movement=camera_movement,
        camera_shot=camera_shot, lens_mm=lens_mm, aspect_ratio=aspect_ratio,
        duration=duration, resolution=resolution, seed=seed, no_audio=no_audio,
    )
