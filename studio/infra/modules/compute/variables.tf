...
variable "openrouter_token_parameter" {
  description = <<-EOT
    Name of the SSM SecureString holding the OpenRouter API key, reaching the
    Lambda as `STUDIO_OPENROUTER_TOKEN_PARAMETER`. Same rules as the three
    above: a NAME, read at call time, empty where a developer's machine
    supplies `OPENROUTER_API_KEY` directly.
  EOT
  type        = string
  default     = ""
}
