...
from tests.unit.test_runs import _approve, _create, _project
...
def test_an_edit_moves_it(empty_api):
    ...
    duplicate of one that no longer exists — the same reasoning that makes an
    edit drop the approval.
    """
    project = _project(empty_api)
    run = _create(empty_api, project, plan={"params": {}, "prompt": "a wave"})
...
def test_a_submitted_run_keeps_the_fingerprint_it_was_approved_with(empty_api):
