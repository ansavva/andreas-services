...
def model_readme(name: str):
    ...
    if entry is not None and registry.provider_of(entry) != registry.REPLICATE:
        # Neither Runpod, fal nor OpenRouter publishes a README this can fetch.
        # The entry's `note` is the prose studio wrote about it, and is what
        # `add-model` would have read.
        readme = f"# {model_id}\n\n{entry.get('note') or ''}\n"
