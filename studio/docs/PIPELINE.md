...
└── src/                           ← src layout, deliberately (see below)
    └── studio_pipeline/
        ...
        │   ├── projects.py
        │   ├── subjects.py        a character OR a location: the bible, the
        │   │                     pools, the selection, the twelve commands
        │   ├── characters/       base.py profile.py refs.py pools.py cli.py
        │   ├── locations.py      `studio location` — subjects.py bound to a room
        │   ├── curate.py  contact_sheet.py
        │   ├── phrasebook.py  prompt.py
        │   ├── templates.py       move the template library between stacks
        │   └── templates/profile.yaml  templates/location.yaml
        │
...
| `studio-media-gpt-image-2` | `openai/gpt-image-2` — **the default for character frames**. Dense legible text, pixel-exact sizes, references held at high fidelity **automatically**. No transparent background |
| `studio-media-gpt-image-2-5-sunburst` | `openai/gpt-image-2.5-sunburst` — OpenAI's newest, tuned for **editing precision**: targeted changes that keep identity, composition and lighting. Adds `xhigh`/`max` quality; transparent background endorsed. Priced per quality tier and `auto` bills at the `xhigh` rate, so the registry defaults `quality` to `medium` |
| `studio-media-gpt-image-2-5-flare` | `openai/gpt-image-2.5-flare` — Sunburst's **fast** sibling: identical schema, prices and defaults, tuned for speed and high volume rather than editing precision. Drafts and iteration |
...
| `studio-media-seedance-2-5` | `bytedance/seedance-2.5` — the successor: better dialogue timing and lip-sync, 4–30 s in one pass, ≤30 reference images, a seed. On Replicate **480p/720p only** ($0.1028/s, $0.2312/s; ~×4 with a reference video). Start frame still excludes references |
...
| `studio-media-ltx-2-3-i2v-lora` | `fal/fal-ai/ltx-2.3-22b/image-to-video/lora` — Lightricks' LTX-2.3 22B image-to-video on **fal**, **loading your own LoRA**: one `loras` list of `{path, scale}` (LTX is one transformer, so a checkpoint is one file). A LoRA is a **send** (role `lora`, `.safetensors` node, `--lora-key`) presigned at submit, never a URL; `lora_scale` is studio's one number for every `scale`, declared in the entry's `input` block and overlaid on the live schema. A required start frame (`image_url`), an optional end frame, `video_size: auto` follows the frame; length is `num_frames` at 24 fps (121 = 5 s). Audio and prompt expansion off by default. $0.001805 per megapixel of output (~$0.20 for 5 s at 720p). Where an LTX-2.3 character LoRA lands. `ltx-2` / `ltx-2-19b` on fal are deprecated since 2026-08-15 |
| `studio-media-wan-2-2-i2v-studio` | `runpod/<endpoint id>` — Wan 2.2 image-to-video on **studio's own Runpod serverless endpoint** (`provider: runpod` still — the client is unchanged; `worker/wan22/` is the handler, ComfyUI on the stock worker-comfyui image, fp16 weights on a network volume). The same LoRA sends as `wan-2.2-i2v-lora`, plus what the public endpoint refuses: an **end frame** (`end_image`, first-last-frame conditioning), `num_frames` (4k+1, up to 161), `fps`, `width`/`height` (unset follows the still's orientation at 720p), `negative_prompt`, `steps`, `guidance`, `shift`, `seed`. **Priced by worker time**, not per clip ($1.66 measured for 121 frames on the serverless H100; `mode: fast` about a third), reported as `output.cost` by the worker itself. The entry's `output_grant` is what makes `dispatch` mint a PUT/GET pair on `scratch/<run>/` for the mp4; the closing path files it like any Runpod clip. `scripts/runpod-endpoint-up.sh` and `scripts/runpod-endpoint-down.sh` own the endpoint |
...
| `studio-media-lora-train` | `runpod-pod/ai-toolkit-wan22-14b` — **a trainer, not a model** (`provider: runpod-pod`, `kind: training`). Rents a Runpod GPU pod from the official ai-toolkit image, trains a Wan 2.2 14B LoRA pair on the character's images tagged `dataset` (captions = descriptions, trigger in front), files every checkpoint pair under `<character>/models/` **as it lands** (the pod reports progress on the signed callback URL after each upload; the run stays `running` and its `outputs` grow), draws one still per `sample_prompts` entry at every save point and files it under `<character>/models/samples/` beside the pair (`[]` for none; on the `i2v` base each prompt re-renders one dataset photo, dealt round-robin, because the I2V model samples from an image or not at all — the run's `training.samples` say which), records pod-hours × rate, terminates the pod. `max_hours` is enforced on the pod itself |
| `studio-media-ltx-2-3-lora-train` | `runpod-pod/ai-toolkit-ltx23-22b` — **a trainer, not a model**, the second on ai-toolkit: the same job script with `arch: ltx23` in the manifest writes the LTX-2.3 config (the 22B mono checkpoint and its Gemma 3 text encoder, both quantised to fit an 80 GB card; `weighted` timesteps; latents cached). **One file per checkpoint**, `<stem>_<step>.safetensors` and a bare `<stem>` at the end — `training.expected_files` takes the trainer's experts, `()` here. Samples are text-only stills at guidance 3, so they show whether the face travels. No `base` knob; `steps` defaults to 1500 and `gpu` to `h100` |
| `studio-media-hunyuan-video-lora-train` | `runpod-pod/musubi-hunyuan-video` — **a trainer, not a model**, on kohya-ss/musubi-tuner off a plain PyTorch image, because ai-toolkit has no HunyuanVideo arch: its own job script (`training.MUSUBI_JOB_SCRIPT`) installs the trainer, fetches the official 720p transformer and VAE and ComfyUI's text encoders, caches latents and text embeddings, trains with musubi's HunyuanVideo network trainer; the shared uploader renames musubi's `<stem>-step<8 digits>` to studio's `<stem>_<step>` and converts each file to the diffusers/ComfyUI key layout on the way up. No samples (`sample_prompts` must be `[]`). **Not run on a real pod, and no hosted endpoint loads its LoRA into image-to-video** (checked 2026-09-20), so nothing in studio can use what it trains yet |
...
| `studio-media-location` | Manage the places a frame is shot in — a room, a set, a street — the same record as a character with a bible of what is fixed, where the light comes from and which camera vantages exist; `--location` on `studio run` binds one beside the cast |
...
| `subjects.py` | **A SUBJECT — a character or a location — and the one command tree both get.** The two are the same rows with a different bible, so everything about a subject is written once here and bound twice: `resolve` (an id is one call, a name is a listing plus a match), the bible's local `edit` round trip whose conflict check is a `rev` sent with the write (compare-and-swap, not check-then-write), `selection_nodes` (resolved by the API, `--slots` applied here), the pool helpers, and `commands(subject)` — the twelve `studio <cli>` commands built from a `Subject`: API segment, id prefix, yaml template, the keys that template has, and the words a message uses. |
| `characters/` | The character record: `base` declares the character `Subject` (`templates/profile.yaml`, `char-`, the four conventional pools) and builds `COMMANDS` once; `profile`, `refs` and `pools` keep the names the rest of the pipeline and its tests were written against — `CHARACTER.load_profile`, `resolve`, `selection_nodes` — each the generic function with the character subject supplied. `cli` assembles the group. |
| `locations.py` | `studio location` — the location `Subject` (`templates/location.yaml`, `loc-`, `reference/corpus/archive`) and the group `subjects.group` builds from it. A location's bible is `space`, `dressing`, `lighting`, `palette`, `rendering` (with named camera `vantages`) and `consistency`; its group tags are the vantages (`wide`, `reverse`, `detail`). `engine/refs.py` resolves one for `studio run --location`. |
...
| `refs.py` | Character and location reference selection and project input pool → **node ids**. Selection itself is `GET /api/characters/<id>/selection`, so the CLI and the SPA cannot disagree about which images a generation saw; what is here is the translation, and the over-cap refusal that names the commands which narrow a set. |
