...
| `studio-media-krea-2-large` | `krea/krea-2-large` — Krea's flagship still model: photorealism and expressive styles, `creativity` from `raw` to `high`, the only image model with a **seed**. `style_reference_images` (≤10) transfers a **look, not a likeness** — not for holding a character on-model |
