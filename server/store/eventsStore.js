...
const mongoDbName = process.env.MONGODB_DB || 'xronos';
const mongoCollection = process.env.MONGODB_COLLECTION || 'events';
...
class FileEventsStore extends InMemoryQueryableStore {
  constructor() {
    super();
  ...
  async load() {
    ...
    try {
      const fileContents = await readFile(this.dataFile, 'utf-8');
  ...
  async persist() {
    await writeFile(this.dataFile, JSON.stringify(this.events, null, 2));
  }
...
class MongoEventsStore extends InMemoryQueryableStore {
  ...
  async load() {
    ...
    const seedsContents = await readFile(SEED_FILE, 'utf-8');
    const seeds = JSON.parse(seedsContents);
    const existingCount = await this.collection.estimatedDocumentCount();
  ...
  async createEvent(payload) {
    ...
    this.events = sortEvents([...this.events, newEvent]);
    await this.collection.insertOne({ _id: newEvent.id, ...newEvent });
    return newEvent;
  }
...
async function createMongoEventsStore() {
  ...
  const store = new MongoEventsStore(client, collection);
  await store.load();
  return store;
}

async function createFileEventsStore() {
  const store = new FileEventsStore();
  await store.load();
  return store;
}
...
export async function createEventsStore() {
  if (mongoUri) {
    return createMongoEventsStore();
  }

  return createFileEventsStore();
}
