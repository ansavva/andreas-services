    ...
    <ol className="items-center w-full space-y-4 sm:flex sm:space-x-8 sm:space-y-0 rtl:space-x-reverse">
      {steps.map((step, index) => {
        ...
        return (
          <li
            key={index}
            className={`flex items-center ${stepClass} space-x-2.5 rtl:space-x-reverse`}
          >
            <span
              className={`flex items-center justify-center w-8 h-8 border rounded-full shrink-0 ${stepClass}`}
            >
              {isCompleted ? "✓" : index + 1}
            </span>
            <span>
