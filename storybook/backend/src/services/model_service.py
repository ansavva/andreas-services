...
class ModelService:
    ...
    def train(self, project_id: str, directory: str) -> str: 
        model_name = self.__get_model_name(project_id)
        try:
            # Try to get the model by name
        ...
        zip_file_buffer = self.image_service.create_zip(project_id, directory)
    ...
    def generate(self, prompt: str, project_id: str) -> File:
        model_name = self.__get_model_name(project_id)
        model = replicate.models.get(f"ansavva/{model_name}")
        ...
        if response.status_code == 200:
            ...
            # Call the upload_file method to upload the image to S3
            image_file = self.image_service.upload_file(project_id, "generated_images", file, "out.jpg")
            return image_file
        else:
