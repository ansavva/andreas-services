...
# classroom frontends depend on. Installing it requires a token carrying the
...
#
# The frontends' committed .npmrc already reads the token from an env var:
...
# NODE_AUTH_TOKEN. It NEVER writes a token into a committed file.
#
...
SCOPE="read:packages"
PKG_PATH="@ansavva%2Fdesign-system"
...
emit_result() {
  ...
  case "$MODE" in
    ...
    *)
      ok "GitHub Packages read access confirmed for @ansavva/design-system."
      ...
      log "Then 'npm ci' in humbugg/marketing, humbugg/app, website/frontend and classroom/frontend will resolve the package."
      ;;
  esac
}
