#!/usr/bin/env bash
#
# Idempotently provision the Cognito admin user for the scout admin console.
#
...
#   SCOUT_ADMIN_EMAIL     admin login email (used as the username)
#   SCOUT_ADMIN_PASSWORD  permanent password to converge to
#
...
# the same two in website's script, so one `export ADMIN_PASSWORD` in a
...
# to exit early on an existing user, which meant rotating SCOUT_ADMIN_PASSWORD and
...
# on the next run. Change it in SCOUT_ADMIN_PASSWORD instead, or pass --no-converge.
#
# The password is set `--permanent`, so there is no FORCE_CHANGE_PASSWORD
# challenge for SRP sign-in to get stuck behind.
...
ADMIN_EMAIL="${SCOUT_ADMIN_EMAIL:-${ADMIN_EMAIL:-}}"
ADMIN_PASSWORD="${SCOUT_ADMIN_PASSWORD:-${ADMIN_PASSWORD:-}}"
...
PASSWORD_VAR="SCOUT_ADMIN_PASSWORD"
if [ -z "${SCOUT_ADMIN_PASSWORD:-}" ] && [ -n "${ADMIN_PASSWORD:-}" ]; then
  PASSWORD_VAR="ADMIN_PASSWORD"
fi
: "${ADMIN_EMAIL:?SCOUT_ADMIN_EMAIL is required}"
...
# read-only question.
[ "$CHECK" = true ] || : "${ADMIN_PASSWORD:?SCOUT_ADMIN_PASSWORD is required}"
