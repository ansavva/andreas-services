...
class TestStore(unittest.TestCase):
    ...
    def test_store_uses_local_dynamodb_endpoint(self):
        with unittest.mock.patch.dict(os.environ, {"DYNAMODB_ENDPOINT_URL": "http://localhost:8001"}):
            dynamodb_adapter._dynamodb = None
            try:
                self.assertEqual(
                    dynamodb_adapter.resource().meta.client.meta.endpoint_url,
                    "http://localhost:8001",
                )
            finally:
                dynamodb_adapter._dynamodb = None
