...
def _extract_one(pages):
    return [{"title": "Extracted", "pages": len(pages)}]
...
class TestPipeline(unittest.TestCase):
    ...
    def test_preview_persists_nothing(self):
        result = pipeline.preview(self.src, fetch_fn=_fetch_fn, extract_fn=_extract_one)
    ...
    def test_execute_run_stores_artifacts_and_finishes(self):
        run = pipeline.execute_run(self.src, runs.TRIGGER_SCHEDULED,
                                   fetch_fn=_fetch_fn, extract_fn=_extract_one)
        ...
        # Root + linked page are in S3 and readable.
    ...
    def test_execute_run_records_error_on_root_failure(self):
        ...
        run = pipeline.execute_run(self.src, runs.TRIGGER_MANUAL,
                                   fetch_fn=boom, extract_fn=_extract_one)
