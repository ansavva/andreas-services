...
def message(message_id="hmb_invitation_123", attachments=None):
    return {
        "schema_version": 1,
...
def test_development_submission_is_captured_once(gateway):
    ...
    retry = client.post("/v1/services/humbugg/messages", json=message())
...
def test_development_submission_rejects_conflicting_id(gateway):
    ...
    response = client.post(
        "/v1/services/humbugg/messages", json=message() | {"subject": "Changed"}
    )
...
def test_development_attachment_upload_and_capture(gateway):
    ...
    registration = client.post(
        ...
        json={
            "schema_version": 1,
            "application_message_id": "hmb_invitation_attachment",
            "file_name": "guide.pdf",
