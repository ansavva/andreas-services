...
resource "aws_lambda_function" "ingress" {
  ...
  timeout       = 30
  memory_size   = 512
  image_config { command = ["mailer.handlers.ingress.handler"] }
  environment {
    variables = {
...
resource "aws_lambda_function" "sender" {
  ...
  image_config { command = ["mailer.handlers.sender.handler"] }
  environment {
    variables = {
      MAILER_CONTENT_BUCKET        = aws_s3_bucket.content.id
      MAILER_MESSAGES_TABLE        = aws_dynamodb_table.messages.name
...
resource "aws_lambda_function" "feedback" {
  ...
  timeout       = 120
  memory_size   = 512
  image_config { command = ["mailer.handlers.feedback.handler"] }
  environment {
    variables = {
      MAILER_CONTENT_BUCKET                  = aws_s3_bucket.content.id
      MAILER_MESSAGES_TABLE                  = aws_dynamodb_table.messages.name
