...
from mailer.config import configuration_set_registry, load_service_registry
from mailer.errors import RetryableError
from mailer.models import recipient_hash
from mailer.status import STATUS_RANK, publish, ttl_90_days
...
def _suppresses_recipient(payload: dict[str, Any], status: str) -> bool:
    if status == "complaint":
        return True
    return status == "bounce" and payload.get("bounce", {}).get("bounceType") == "Permanent"
...
def _suppress(store: AwsStore, recipients: list[str], reason: str, event_id: str) -> None:
...
def _process(record: dict[str, Any], registry: dict, config_sets: dict[str, str]) -> None:
    ...
    _metric(status.replace("_", "-").title().replace("-", ""), service_id=service.service_id)
    publish(
        service,
        status_event(
            service,
