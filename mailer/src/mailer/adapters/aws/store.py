...
def _iso(timestamp: int | None = None) -> str:
    return datetime.fromtimestamp(timestamp or _now(), UTC).isoformat().replace("+00:00", "Z")
...
class AwsStore:
    ...
    def attachment_records(
        ...
        for reference in message.attachments:
            ...
            if any(metadata.get(key) != value for key, value in expected_metadata.items()):
                raise ValidationError("attachment does not belong to this service and message")
            try:
                ...
                raise ValidationError("attachment registration metadata is invalid") from exc
    ...
    def admit_message(self, service: ServiceConfig, message: MessageRequest) -> None:
        record_key = self.message_key(service.service_id, message.application_message_id)
        ...
        manifest_key = (
            f"requests/{service.service_id}/{message.application_message_id}/manifest.json"
        )
