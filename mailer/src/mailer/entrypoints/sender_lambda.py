...
from mailer.adapters.aws.status import publish
from mailer.adapters.aws.store import AwsStore
from mailer.core.config import ServiceConfig
from mailer.core.errors import AttachmentBlockedError, RetryableError, ValidationError
from mailer.core.mime import AttachmentContent, build_message
from mailer.core.models import MessageRequest, recipient_hash
...
def _enabled(service: ServiceConfig) -> bool:
    ...
    return response["Parameter"]["Value"].strip().lower() in {"true", "1", "yes", "enabled"}
...
def _attachments(store: AwsStore, manifest: dict[str, Any]) -> list[AttachmentContent]:
    ...
    for value in manifest.get("attachments", []):
        ...
        tags = store.s3.get_object_tagging(Bucket=store.bucket, Key=key).get("TagSet", [])
        scan = next(
            (item["Value"] for item in tags if item["Key"] == "GuardDutyMalwareScanStatus"),
            None,
        )
        if scan is None:
        ...
        if hashlib.sha256(data).hexdigest() != value["sha256"]:
            raise AttachmentBlockedError("attachment checksum changed after registration")
        contents.append(
            AttachmentContent(
...
def _send_record(record: dict[str, Any], registry: dict[str, ServiceConfig]) -> None:
    ...
    try:
        ...
        if code in {"TooManyRequestsException", "ThrottlingException", "ServiceUnavailable"}:
    ...
    store.s3.delete_objects(Bucket=store.bucket, Delete={"Objects": keys, "Quiet": True})
