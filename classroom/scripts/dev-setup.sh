#!/usr/bin/env bash
#
# dev-setup.sh — make this checkout runnable: the per-machine dev.env, backend
# deps, frontend deps.
#
...
# ---------------------------------------------------------------------------
# 1. ~/.config/andreas-services/classroom/dev.env, FROM THIS MACHINE'S DEV STACK.
#
#    One file for everything local — the frontend's VITE_* values and the dev
#    account — outside the repo, so it survives `git clean` and a fresh
#    worktree, and holds a password where no `git add -f` reaches it.
#    `dev-up.sh` exports the VITE_* prefix before Vite starts.
#
...
if dev_stack="$(
  ...
  # `frontend/.env.local`, which this replaced, is imported once and removed.
  previous="$(mktemp)"
  chmod 600 "$previous"
  ...
  if [ -f "$FRONTEND/.env.local" ]; then
    while IFS= read -r line; do
      ...
      ensure_env "$previous" "${BASH_REMATCH[1]}" "${BASH_REMATCH[2]}"
    done < "$FRONTEND/.env.local"
    rm -f "$FRONTEND/.env.local"
    log "imported frontend/.env.local into $DEV_ENV_FILE and removed it"
  fi
  written=""
  rendered="$(mktemp)"
  chmod 600 "$rendered"
  ...
  line "# Classroom local development — the one file. classroom/dev.env.sample documents it."
  ...
  line "# against production. The app client registers $SPA_ORIGIN."
  gen VITE_API_URL "http://localhost:8001/api"
  ...
  keep CLASSROOM_DEV_USER_EMAIL
  keep CLASSROOM_DEV_USER_PASSWORD
  ...
  log "wrote $DEV_ENV_FILE (dev pool $POOL_ID, sign-in at $AUTH_DOMAIN)"
  ...
  warn "no dev stack readable — skipping $DEV_ENV_FILE."
