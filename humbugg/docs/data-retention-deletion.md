...
Clears the caller's own wishlist, avoidances, mailing address, purchase claims, gift progress and question threads for that exchange. The membership
...
| **Gift progress** (#132) | **Deleted / anonymized** with the membership row it lives on, and — for the "it arrived" the member left on somebody else's row — cleared explicitly on every removal path, because it would otherwise outlive them on a row the sweep does not touch. |
...
| **Memberships in groups the user only participates in — completed draw** | **Anonymized, not deleted.** The row is kept (its `member_id` is referenced by the completed draw) but `user_id` is repointed to an irreversible pseudonym and the display name, wishlist, avoidances, address, purchase claims, gift progress and question threads are erased. This preserves the integrity of a draw other people already rely on while removing the personal data behind it. |
