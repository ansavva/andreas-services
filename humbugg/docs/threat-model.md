...
| Gift progress (stage / received) | Medium — a stage is harmless, a pairing is not | `humbugg-prod-groupmembers`; the roll-up an organizer reads is **counts only** |
...
- **Mitigation [NOW]:**
  - The mapping lives in `humbugg-prod-draws`, separate from group data, and is never returned wholesale.
...
- **Audit write:** a record is written to `humbugg-prod-audit-events` with `group_id`, a time-ordered
...
- **Suspected assignment leak / misuse of reveal:** query `humbugg-prod-audit-events` by `group_id` for
