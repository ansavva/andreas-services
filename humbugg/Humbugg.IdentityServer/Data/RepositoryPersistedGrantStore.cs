...
namespace Humbugg.IdentityServer.Data
{
    public class RepositoryPersistedGrantStore : IPersistedGrantStore
    {
        ...
        public Task<IEnumerable<PersistedGrant>> GetAllAsync(PersistedGrantFilter filter)
        {
           ...
           return Task.FromResult(_repository.Where<PersistedGrant>(
                i => (string.IsNullOrWhiteSpace(filter.SubjectId) || i.SubjectId == filter.SubjectId) &&
                     (string.IsNullOrWhiteSpace(filter.ClientId) || i.ClientId == filter.ClientId) &&
                     (string.IsNullOrWhiteSpace(filter.Type) || i.Type == filter.Type) &&
                     (string.IsNullOrWhiteSpace(filter.SessionId) || i.SessionId == filter.SessionId)).AsEnumerable());
        }
        ...
        public Task RemoveAllAsync(PersistedGrantFilter filter)
        {
            ...
            _repository.Delete<PersistedGrant>(
                i => (string.IsNullOrWhiteSpace(filter.SubjectId) || i.SubjectId == filter.SubjectId) &&
                     (string.IsNullOrWhiteSpace(filter.ClientId) || i.ClientId == filter.ClientId) &&
                     (string.IsNullOrWhiteSpace(filter.Type) || i.Type == filter.Type) &&
                     (string.IsNullOrWhiteSpace(filter.SessionId) || i.SessionId == filter.SessionId));
