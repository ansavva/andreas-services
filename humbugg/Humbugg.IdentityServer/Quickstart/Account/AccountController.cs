...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            // build a model so we know what to show on the login page
            var vm = await BuildSignUpViewModelAsync(returnUrl);
            return View(vm);
        }
        ...
        /// <summary>
        /// Handle postback from user SignUp
        /// </summary>
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SignUp(SignUpInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                _loginService.CreateProfile(user);
                // validate username/password against in-memory store
                await _events.RaiseAsync(new UserLoginSuccessEvent(user.Email, user.Id, user.Email, clientId: context?.Client.ClientId));
            ...
            // something went wrong, show form with error
            var vm = await BuildSignUpViewModelAsync(model);
            return View(vm);
        }
