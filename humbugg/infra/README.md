...
`humbugg-<env>-audit-events` is the standard, append-only audit log for security- and
...
never expired automatically (unlike `humbugg-<env>-email-messages`, which expires after 90 days).
...
`humbugg-<env>-analytics-events` stores the **privacy-safe product-analytics funnel** — a stream separate
...
`humbugg-<env>-email-messages`. The table records normalized state and expires records
after 90 days.
