"""SES inbound receipt-rule handler that forwards support mail.

Flow
----
1. SES accepts mail for ``support@humbugg.com``, stores the raw MIME in S3, and
   invokes this Lambda asynchronously with the SES receipt event.
...
3. It fetches the raw MIME from S3, parses it with the stdlib ``email`` package,
   ...
   copied across.
4. The forwarded message is sent via SES from ``no-reply@humbugg.com`` to the
...
# comes back through support@ can be detected and dropped.
FORWARD_MARKER_HEADER = "X-Humbugg-Forwarded"
...
class Settings:
    ...
    def from_env(cls, *, ssm_client=None) -> "Settings":
        ...
        allowed = cls._clean_list(
            os.environ.get("ALLOWED_RECIPIENTS", "support@humbugg.com")
        )
        own = cls._clean_list(os.environ.get("OWN_DOMAINS", "humbugg.com"))

        return cls(
            inbound_bucket=bucket,
            inbound_prefix=os.environ.get("INBOUND_PREFIX", "inbound/"),
            from_address=os.environ.get("FROM_ADDRESS", "no-reply@humbugg.com"),
            ...
            max_attachment_bytes=int(
                os.environ.get("MAX_ATTACHMENT_BYTES", "6000000")
            ),
        )
...
def _resolve_forward_to(*, ssm_client=None) -> str:
    ...
    ``SUPPORT_FORWARD_TO`` (direct) wins for local/test use; production wires
    ``SUPPORT_FORWARD_TO_PARAM`` to an SSM SecureString path.
    """

    direct = os.environ.get("SUPPORT_FORWARD_TO", "").strip()
    if direct:
        return direct

    param_name = os.environ.get("SUPPORT_FORWARD_TO_PARAM", "").strip()
    if not param_name:
        raise ConfigError(
            "No forward destination configured: set SUPPORT_FORWARD_TO or "
            "SUPPORT_FORWARD_TO_PARAM"
        )
...
def _iter_safe_attachments(
    ...
    for part in attachments:
        ...
        kept.append((maintype or "application", subtype or "octet-stream", filename, payload))
        used += size
...
def build_forwarded_message(raw_bytes: bytes, settings: Settings) -> EmailMessage:
    ...
    header_block = (
        "Forwarded message received at support@humbugg.com\n"
        f"From:    {orig_from}\n"
        f"To:      {orig_to}\n"
        f"Date:    {orig_date}\n"
    ...
    message["Subject"] = _sanitize_header(f"[Humbugg support] {orig_subject}")
...
def _strip_attachments(
    ...
    for key in ("From", "To", "Reply-To", "Subject", FORWARD_MARKER_HEADER, "Auto-Submitted"):
...
def process_record(
    ...
    *,
    ...
    logger.info("message %s: forwarded to private support inbox", message_id)
    return "forwarded"
...
def lambda_handler(event: dict, context: Any = None) -> dict:
    ...
    for record in _records(event):
        results.append(
            process_record(
                record, settings, s3_client=s3_client, ses_client=ses_client
            )
        )
