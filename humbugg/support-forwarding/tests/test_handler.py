"""Unit tests for the support-mail forwarder.
...
from support_forwarder import handler
from support_forwarder.handler import (
    ConfigError,
    Settings,
    TransientError,
...
def settings() -> Settings:
    return Settings(
        inbound_bucket="humbugg-support-inbound-test",
        inbound_prefix="inbound/",
        from_address="no-reply@humbugg.com",
        forward_to=FORWARD_TO,
        allowed_recipients=frozenset({"support@humbugg.com"}),
        own_domains=frozenset({"humbugg.com"}),
...
def make_raw(
    *,
    ...
    to: str = "support@humbugg.com",
    subject: str = "Help please",
    body: str = "My gift exchange is broken.",
    html: str | None = None,
    ...
    *,
    ...
    return {
        "ses": {
            ...
            "receipt": {
                "recipients": recipients or ["support@humbugg.com"],
...
def test_product_email_still_from_no_reply(settings):
    ...
    assert ses.sent[0]["Source"] == "no-reply@humbugg.com"
...
def test_drops_loop_from_own_domain(settings):
    record = make_record(sender="no-reply@humbugg.com", from_header="no-reply@humbugg.com")
...
def test_drops_wrong_recipient(settings):
    record = make_record(recipients=["someone-else@humbugg.com"])
    status, ses = _run(record, settings, make_raw())
...
def test_from_env_reads_ssm_secret(monkeypatch):
    ...
    monkeypatch.delenv("SUPPORT_FORWARD_TO", raising=False)
    monkeypatch.setenv("SUPPORT_FORWARD_TO_PARAM", "/humbugg/prod/support-forward-to")
...
def test_from_env_direct_env_wins(monkeypatch):
    monkeypatch.setenv("INBOUND_BUCKET", "b")
    monkeypatch.setenv("SUPPORT_FORWARD_TO", FORWARD_TO)
    s = Settings.from_env()
    assert s.forward_to == FORWARD_TO
...
def test_from_env_missing_secret_raises(monkeypatch):
    monkeypatch.setenv("INBOUND_BUCKET", "b")
    monkeypatch.delenv("SUPPORT_FORWARD_TO", raising=False)
    monkeypatch.delenv("SUPPORT_FORWARD_TO_PARAM", raising=False)
...
def test_build_forwarded_message_rejects_unparseable(settings, monkeypatch):
    ...
    assert msg["From"] == "no-reply@humbugg.com"
